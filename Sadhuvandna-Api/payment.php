...
if(empty($uesr_email) && $sender_id > 0) {
    // App Flow: Fetch user details via Marriage Profile Logic
    // We assume sender_id is Marriage Profile ID.
    $mp = $con->query("SELECT user_id, full_name, email, phone FROM tbl_marriage_profiles WHERE id='$sender_id' LIMIT 1")->fetch_assoc();
    if($mp){
        $res['name'] = $mp['full_name'];
        $res['mobile'] = $mp['phone'];
        $uesr_email = $mp['email'];
        // Fix: If email is empty, use a placeholder or handle gracefully
        if(empty($uesr_email)) $uesr_email = "user_{$mp['user_id']}@sadhuvandna.app"; 
        
        $logged_user_id = $mp['user_id'];
    }
} else {
    // Website Flow
