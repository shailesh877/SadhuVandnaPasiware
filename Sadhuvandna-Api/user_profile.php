...
<main class="flex-1 px-3 md:px-10 py-15 bg-white md:ml-20 mb-14 md:mb-0">
  ...
  <?php if (!empty($user['cover_photo'])): ?>
    <div class="relative w-full h-52 md:h-64 rounded-xl overflow-hidden shadow">
      <img src="uploads/photo/<?php echo htmlspecialchars($user['cover_photo']); ?>" class="w-full h-full object-cover" onclick="openImageModal(this.src)">
    </div>
  <?php endif; ?>
  ...
  <div class="bg-white rounded-xl shadow-lg border border-orange-200 mt-[-3rem] md:mt-[-4rem] relative z-10 px-6 py-5">
    <div class="flex flex-col md:flex-row items-center md:items-end gap-4">
      <img src="uploads/photo/<?php echo htmlspecialchars($user['profile_photo']); ?>" class="w-28 h-28 rounded-full border-4 border-orange-300 shadow-lg bg-white" onclick="openImageModal(this.src)">
      <div class="flex flex-col md:flex-row justify-between w-full md:items-end" >
        <div>
          <h1 class="text-2xl font-bold text-orange-700"><?php echo htmlspecialchars($user['name']); ?></h1>
          <?php if (!empty($user['about'])): ?>
            <p class="text-gray-700 italic mt-1"><?php echo htmlspecialchars($user['about']); ?></p>
          <?php endif; ?>
          <p class="text-sm text-gray-500 mt-1">Joined on: <?php echo date("d M, Y", strtotime($user['date'])); ?></p>
        ...
        <?php if($logged_email === $user['email']): ?>
          <a href="edit_profile.php" class="bg-orange-500 text-white px-4 py-2 rounded-lg shadow hover:bg-orange-700">Edit Profile</a>
        <?php endif; ?>
      </div>
    </div>
  </div>
      ...
      <?php if($family_members->num_rows > 0): ?>
        ...
        <?php while($fm = $family_members->fetch_assoc()): ?>
          <div class="flex gap-3 items-center p-2 bg-white rounded-lg border border-orange-100 shadow-sm">
            ...
            <div class="flex-1">
              <div class="font-bold text-orange-700"><?= htmlspecialchars($fm['name']) ?></div>
              <div class="text-xs text-gray-600"><?= htmlspecialchars($fm['relation']) ?> | <?= htmlspecialchars($fm['gender']) ?></div>
              ...
              <div class="text-xs text-gray-600"><?= htmlspecialchars($fm['occupation']) ?></div>
      ...
      <a href="view_marriage_profile.php?id=<?= $marriage_profile['id'] ?>" 
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <div class="text-sm text-gray-600">
              <?= htmlspecialchars($marriage_profile['city']) ?>, <?= htmlspecialchars($marriage_profile['caste']) ?>
            </div>
...
.custom-scroll::-webkit-scrollbar-track { background: #fdecd5; }
</style>
  ...
  <div class="bg-white w-[92%] max-w-md rounded-xl shadow-2xl overflow-hidden border border-orange-200">
    ...
    <div class="bg-gradient-to-r from-orange-600 to-orange-500 p-5 text-white relative">
      ...
      <button id="closeModal"
              class="absolute top-3 right-4 text-white/90 text-2xl hover:text-white font-bold">
        &times;
      </button>
      ...
      <div class="flex items-center gap-4">
        ...
        <?php else: ?>
            <div class="w-16 h-16 rounded-lg bg-white/30 flex items-center justify-center text-white text-3xl font-bold shadow">
