...
$orderId = $_SESSION['anchor_orderid'] ?? '';
$amount = $_SESSION['anchor_amount'] ?? 100;
...
if (isset($_POST['razorpay_payment_id']) && isset($_POST['razorpay_signature']) && isset($_POST['razorpay_order_id'])) {
    ...
    if ($success) {
        // Insert success payment into wallet with payment_type = 'anchor'
        $result = mysqli_query($con, "
            ...
            VALUES 
            ('$user_id', 0, 0, '$razorpay_id', '$payment_id', '$amount', '$razorpay_signature', 'success', 'anchor', NOW())
        ");
        
        $status_msg = "Success";
        $title = "Payment Successful";
        $detail = "Your payment of ₹" . $amount . " was received. You can now return to the app and submit your news anchor application.";
    } else {
        ...
        mysqli_query($con, "
            ...
            (user_id, sender_id, receiver_id, order_id, payment_id, payment_ammount, payment_signature, status, payment_type, date)
            VALUES 
            ('$user_id', 0, 0, '$razorpay_id', '$payment_id', '$amount', '$razorpay_signature', 'failed', 'anchor', NOW())
        ");
...
<body>
    <div class="card">
        ...
        <h2>' . $title . '</h2>
        <p>' . $detail . '</p>
        <a href="sadhuvandna://anchor_payment_success" class="btn">Return to Linkup App</a>
    </div>
</body>
</html>';
?>
