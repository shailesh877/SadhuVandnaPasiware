...
$profile_id   = intval($_POST['profile_id'] ?? 0);      // jiski status dekhni
$my_profile   = intval($_POST['my_profile_id'] ?? 0);   // meri profile

if (!$profile_id || !$my_profile) {
...
$typ = $con->query("
    ...
    AND target_profile_id='$my_profile'
    LIMIT 1
");

if ($typ && $typ->num_rows > 0) {
    $r = $typ->fetch_assoc();
    $response['is_typing'] = !empty($r['is_typing']);
}

/* ===============================
   3️⃣ Max Seen Message ID
================================ */
$s = $con->query("
    SELECT MAX(id) AS m 
    FROM tbl_messages 
    WHERE sender_id='$my_profile' 
...
/* ===============================
   4️⃣ Incoming Call (to ME)
================================ */
$inc = $con->query("
    SELECT * 
    FROM tbl_calls 
    WHERE receiver_id='$my_profile' 
    AND status='ringing'
...
/* ===============================
   5️⃣ Call Status Update (my call)
================================ */
$my_call = $con->query("
    SELECT * 
    FROM tbl_calls 
    WHERE caller_id='$my_profile'
