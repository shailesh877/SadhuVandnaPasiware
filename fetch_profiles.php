...
if((!$result || $result->num_rows==0) && $offset == 0){
    echo "<div class='text-center text-gray-500 mt-10 w-full col-span-full'>No profiles found.</div>";
    exit;
}
...
<div class="bg-white rounded-2xl shadow-xl p-6 flex flex-col items-center border border-orange-200 gap-2">
    <img src="<?php echo $photo ?>" class="w-24 h-24 rounded-full border-4 border-orange-400 object-cover mb-2">
    <div class="font-bold text-orange-700 text-lg text-center"><?php echo htmlspecialchars($row['full_name']) ?></div>
    <div class="text-gray-600 text-sm text-center"><?php echo $row['age'] ?> yrs | <?php echo htmlspecialchars($row['education']) ?></div>
    <div class="text-gray-500 text-xs text-center"><?php echo htmlspecialchars($row['city']) ?></div>
    <span class="px-2 py-0.5 bg-orange-100 text-orange-700 rounded-full text-xs mb-2"><?php echo htmlspecialchars($row['status']) ?></span>

    <div class="flex gap-2 w-full">
        <a href="view_marriage_profile.php?id=<?php echo $row['id'] ?>" class="flex-1 bg-orange-50 border border-orange-300 text-orange-700 font-semibold rounded-lg py-1 text-xs flex items-center justify-center gap-1">
            <i class="fa fa-eye"></i> View
        </a>

        <?php if(!$proposal_status || $proposal_status=='rejected'): ?>
            <a href="send_proposal.php?to=<?php echo $row['id'] ?>&profile_id=<?php echo $row['id'] ?>" class="flex-1 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-lg py-1 text-xs flex items-center justify-center gap-1">
        ...
        <?php elseif($proposal_status=='accepted' || $proposal_status=='friend'): ?>
            <a href="message.php?sender_id=<?php echo $my_profile_id ?>&receiver_id=<?php echo $row['id'] ?>" class="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold rounded-lg py-1 text-xs flex items-center justify-center gap-1">
